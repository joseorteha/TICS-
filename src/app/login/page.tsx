  ...
  useEffect(() => {
    fetch('/api/me')
      ...
      .then(({ status, profileId }: { status: string; profileId?: string }) => {
        if (status === 'has_profile' && profileId) router.replace(`/perfil/${profileId}`)
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="text-center mb-6">
          <div className="flex justify-center mb-3">
            <Image src="/logo-itsz.svg" alt="ITSZ" width={72} height={72} className="object-contain" />
          </div>
          <h1 className="text-lg font-bold text-foreground">TICs para Teachers</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Diagnóstico Digital Docente · ITSZ</p>
        </div>

        {/* Card glass */}
        <div className="glass-card rounded-3xl p-7 space-y-5">
          <div className="text-center">
            <h2 className="text-base font-semibold text-foreground">Acceso institucional</h2>
          ...
          {error === 'dominio_no_permitido' && (
            <div className="flex items-start gap-2.5 p-3 rounded-xl bg-red-50 border border-red-200">
              ...
              <div>
                <p className="text-xs font-medium text-red-700">Correo no permitido</p>
                {email && <p className="text-xs text-red-600 mt-0.5 font-mono">{email}</p>}
                <p className="text-xs text-red-600 mt-0.5">Usa tu correo institucional ITSZ.</p>
              </div>
            </div>
          )}
          {error === 'auth_failed' && (
            <div className="flex items-start gap-2.5 p-3 rounded-xl bg-red-50 border border-red-200">
              <AlertCircle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
              <p className="text-xs text-red-700">Error al autenticar. Intenta de nuevo.</p>
            </div>
          )}
          ...
          <p className="text-center text-xs text-muted-foreground">
            Se abrirá el selector de cuentas Google.<br />
